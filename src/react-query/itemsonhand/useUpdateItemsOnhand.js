...
async function updateItemsOnhand(data) {
  const { itemdata } = await graphQLClient.request(
    gql`
      mutation UpdateItemsOnhand(
        $item_id: ID
        $item_no: String
        $item_branch: String
        $item_qoh_pc: Float
...
export function useUpdateItemsOnhand(data) {
  const queryClient = useQueryClient();

  const { mutate } = useMutation({
    mutationFn: data => updateItemsOnhand(data),
    onSuccess: () => {
      Toast({
        title: 'Items onhand being updated!',
        status: 'success',
        customId: 'itemonhandUpd',
      });
    },
    onError: () => {
      Toast({
